...
#####################
###### wildcard...........wget -r --no-parent -A 'bar.*.tar.gz' http://url/dir/
...
Txyz = grbfile.select(name='Temperature',typeOfLevel='isobaricInhPa')[:]
CLMR = grbfile.select(name='Cloud mixing ratio',typeOfLevel='isobaricInhPa')[:]
IWMR = grbfile.select(name='Ice water mixing ratio',typeOfLevel='isobaricInhPa')[:]
RMR = grbfile.select(name='Rain mixing ratio',typeOfLevel='isobaricInhPa')[:]
SMR = grbfile.select(name='Snow mixing ratio',typeOfLevel='isobaricInhPa')[:]
print('nlevMRs=',len(CLMR),len(IWMR),len(RMR),len(SMR))
RH = grbfile.select(name='Relative humidity',typeOfLevel='isobaricInhPa')[:]
print('nlevRH=',len(RH))
ISAH = grbfile.select(name='ICAO Standard Atmosphere reference height')[:]
print('nlevISAH=',len(ISAH))
