...
#      pressure=plevelTCC
      ...
      Cn2_data_nc.createVariable('Date',np.int,('ntime',),zlib=False)
      Cn2_data_nc.createVariable('lats',np.float32,('nlat','nlon',),zlib=False)
      Cn2_data_nc.createVariable('lons',np.float32,('nlat','nlon',),zlib=False)
      Cn2_data_nc.createVariable('levs',np.float32,('nlev',),zlib=False)
      Cn2_data_nc.createVariable('Cn2',np.float32,('nlev','nlat','nlon',),zlib=False)
      Cn2_data_nc.close()


      Cn2_data = Dataset(Cn2_filename, 'a')
      print('netcdfvarb=',Cn2_data.variables)
      Cn2_data.variables['Date']=int(yrmodahr_str)

      for h in range(nlev_mid):
         Cn2_data.variables['levs'][h]=p_mid[h]
      for i in range(nlat):
         print('i loop,', i)
         for j in range(nlon):
            Cn2_data.variables['lats'][i,j]=lats[i,j]
            Cn2_data.variables['lons'][i,j]=lons[i,j]
            for h in range(nlev_mid):
               Cn2_data.variables['Cn2'][h,i,j]=Cn2[h,i,j] 
      Cn2_data.close()
